...
def face_data(request):
    if(request.method=='GET'):
        articles=SmartEyeData.objects.all()
...
def face_data_detail(request,pk):
    ...
    try:
        article = SmartEyeData.objects.get(pk=pk)

    except SmartEyeData.DoesNotExist:
...
def compare_faces(request):
    ...
    face_datas=SmartEyeData.objects.all()
    ...
    for i in fdata:
        ...
        temp_image=face_recognition.load_image_file(response)
    ...
    # img_path1="http://64.227.178.238/home/django/django_project/media/testvirendra.jpeg" #this thing will be come from the server
    ...
    for(top, right, bottom, left), face_encoding in zip(face_locations, face_encodings):
        ...
        if matches[best_match_index]:
            ...
            new_json={'UserName':known_face_names[best_match_index],'UserID': known_face_id[best_match_index],'TestResponse': matches}
        ...
        if True in matches:
            ...
            new_json={'UserName':known_face_names[first_match_index],'UserID': known_face_id[first_match_index],'TestResponse': matches}
            return Response(new_json)
